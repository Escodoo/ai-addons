...
``agno_chatter_bots.bridge_auth_token`` to the same value as
...
Also ensure ``agno_connector.service_token`` matches Agno
``AGNO_SERVICE_TOKEN`` so personas with Odoo tools can call
``/agno/rpc``.
