...
BYOK values are injected into `_odoo.llm` / `_odoo.embedder` only for bridges
with **Provider** set to `Agno`. Bridges of other providers never receive the
customer API keys.
...
`ai_agno_llm_settings.agno_base_url` when Agno is reachable under another
host (e.g. `http://agno.internal:8000`).

## API keys in ICP

BYOK keys (`ai_agno_llm_settings.api_key`,
`ai_agno_llm_settings.embedder_api_key`) are stored as plain text in
`ir.config_parameter`, which is the usual Odoo pattern for system secrets.
The Settings fields are restricted to `base.group_system`, and keys are
masked (`***`) before being persisted on `ai.bridge.execution` payloads.
Database admins (and anyone with SQL access) can still read ICP values —
treat the database as a trust boundary, same as other Odoo secrets.
