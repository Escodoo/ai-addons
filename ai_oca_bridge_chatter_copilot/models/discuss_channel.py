...
class DiscussChannel(models.Model):
    ...
    def _is_ai_customer_conversation(self):
        """Whether the channel is a conversation held with a customer.

        Customer conversations are not driven by mentions: the assistant is
        expected to answer the customer even when several operators are members
        of the channel. This module only provides the machinery; the glue
        modules declare which channel types qualify.
        """
        self.ensure_one()
        return False

    def _is_ai_conversation_open(self):
        """Whether the conversation still accepts an answer."""
        self.ensure_one()
        return True
    ...
    def _is_ai_bridge_inbound_post(self, **kwargs):
        """Whether the message is being created by the customer side itself.

        Such a post never hands the conversation over to a human, even when the
        technical user writing it is an internal one.
        """
        return False
    ...
    def message_post(self, **kwargs):
        if (
            self._is_ai_customer_conversation()
            and not self._is_ai_bridge_inbound_post(**kwargs)
            and self._is_ai_bridge_operator_post(**kwargs)
        ):
    ...
    def _ai_bridge_note_stays_internal(self):
        ...
        A conversation the customer reads directly must not disclose the
        handoff, so the glue module of such a channel type returns False.
        """
        self.ensure_one()
        return True
    ...
    def _to_store(self, store: Store):
        ...
        for channel in self:
            if not channel._is_ai_customer_conversation():
                continue
            store.add(
                channel,
                {
